...
for e in range(len(arr_e)):
    ...
    for yd in arr_yd:
        ...
        for j in range(len(logMh_array)):
            ...
            ## this is if i use the single average-sigma_d, hence i erase the size variation distribution
            Mean_Sigmad=np.percentile(tauUV,50)/kUV
            #Sigmad_distr = draw_sigma_distribution(mu_sigma=Mean_Sigmad, Mach=100)
            
            #here i keep the variation in size and add on top the one due to clumpy ism
            N_tau = len(tauUV)
            N_samples = 1000
            mu_array = tauUV / kUV       # shape (N_tau,)
            log_mu_array = np.log(mu_array)  # log-space means

            # Broadcast to shape (N_tau, N_samples)
            Mach=100
            log_mu_matrix = log_mu_array[:, np.newaxis]  # (N, 1)
            
            Sigmad_distr = []
            
            for mu_sigma in np.exp(log_mu_array):  # loop over N_tau medians
                samples = draw_sigma_distribution(mu_sigma, Mach, nsamples=N_samples)
                Sigmad_distr.append(samples)

            Sigmad_distr = np.array(Sigmad_distr).flatten()

            
            UV_emitting_Sigmad = np.percentile(Sigmad_distr,10)
            IR_emitting_Sigmad = np.percentile(Sigmad_distr,90)
            
            #print('\nSigma_dust (average, lower UV, higher IR) -->', np.log10(Mean_Sigmad), np.log10(UV_emitting_Sigmad), np.log10(IR_emitting_Sigmad), 'v_rot[km/s]->', compute_vrot(redshift,10**logMh_array[j]))
            #UV emitting ISM fraction, compute its tau to UV photons, to get its atetnuated UV luminosity
            tauUV_UVem = kUV * UV_emitting_Sigmad
            T1500_sf_UV = T_1500_sphere(tauUV_UVem)
            
            #IR emitting ISM fraction, compute its tau to UV photons, to get its IR luminosity
            tauUV_IRem = kUV * (1 - 0.3807) * IR_emitting_Sigmad
            T1500_sf_IR = np.exp(-tauUV_IRem)
            
            # UV side
            LUV_att = T1500_sf_UV * L1500_arr[-1]
            MUV_noatt = L1500_to_MUV_conv(L1500_arr[-1])
            LUV_att_arr = np.atleast_1d(T1500_sf_UV * L1500_arr[-1])
            ...
            # IR side
            nu_1500 = 3e10 / (1500e-8)  # Hz
            LIR_abs = (1 - T1500_sf_IR) * L1500_arr[-1] * nu_1500
            LIR_array_abs.append(np.median(LIR_abs))
            LIR_array_abs_l.append(np.percentile(LIR_abs, 16))
            LIR_array_abs_u.append(np.percentile(LIR_abs, 84))
            
            # Use average Sigmad for both UV and IR (Uniform ISM case)
            tauUV_mean = kUV * Mean_Sigmad
            T1500_sf_mean = T_1500_sphere(tauUV_mean)
            ...
            LUV_att_baseline = T1500_sf_mean * L1500_arr[-1]
            MUV_att_baseline = L1500_to_MUV_conv(LUV_att_baseline)
            LIR_abs_baseline = (1 - T1500_sf_mean) * L1500_arr[-1] * nu_1500
