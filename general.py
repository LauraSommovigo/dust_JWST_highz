...
def write_fits_image(data, header, outname, overwrite=True, verbose=True):
    ...
    if verbose:
        print("Wrote to fits file", outname)
...
def has_unit_initialized(data):
    ...
    try:
        unit = data.unit
        return True if unit is not None else False
    except:
        return False
...
def get_bin_centre(bins, data=None, inds=None, ctype="centre", wtype="confint", **kwargs):
    """What defines the centre of a bin?

    Is it simply the halfway point between the lower and upper
    ...
    """
    if (data is None) and ctype not in ["centre", "logcentre"]:
    ...
    if ctype == "centre":
        ...
        if has_unit_initialized(bincentre):
            ...
            binwidth = 0.5 * np.asarray([binwidth, binwidth])
        ...
        if has_unit_initialized(data):
            data, unit = data.value, data.unit
        else:
            data, unit = data, 1.0
        # Ensure unit of <bins> is equal to that of <data>
        if has_unit_initialized(bins):
            bins = bins.to(unit).value
        else:
            pass
...
def diagonal_lines(axes, factor=2, add=False, **kwargs):
    ...
    for ax in axes:
        ...
        for fac in factor:
            if not add:
                ax.plot(lim, fac * lim, **kwargs)
            else:
                ax.plot(lim, lim + fac, **kwargs)
    return None
...
def set_tickparams(ax, which="both", width=[2, 1], length=[8, 4], direction=["out", "inout"]):
    if not check_if_list(width):
        width = [width, width]
    if not check_if_list(length):
        length = [length, length]
    if not check_if_list(direction):
        direction = [direction, direction]

    if which in ["major", "both"]:
        ax.tick_params(which="major", width=width[0], length=length[0], direction=direction[0])
    if which in ["minor", "both"]:
        ax.tick_params(which="minor", width=width[1], length=length[1], direction=direction[1])
...
def confidence_interval_2D(data, percent=[16, 84]):
    ...
    for i, p in enumerate(percent):
        ind = int(len(data[0]) * float(p) / 100.0 + 0.5)
        myvals[i] = data[:, ind]
